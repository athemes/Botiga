  ...
  window.bhfb = {
    ...
    updateAvailableComponents: function updateAvailableComponents() {
      ...
      if (_this.currentBuilderType === 'header') {
        ...
        $('.botiga-header-builder-available-components').html($('.botiga-bhfb-header .botiga-bhfb-elements-desktop').html()); // Header Mobile Components.
    ...
    elementsButton: function elementsButton() {
      ...
      $(document).on('click', '.botiga-bhfb-element > a', function (e) {
        ...
        if ($(this).closest('#botiga-bhfb-elements').length) {
          _this.elementsButtonAdd(id); // close elements popup.
    ...
    elementsButtonAdd: function elementsButtonAdd(id) {
      ...
      _this.currentRowInput.val(JSON.stringify(current_value)); // Trigger change in the customizer field (desktop).


      _this.currentRowInput.trigger('change'); // Trigger change in the customizer field (mobile).
    ...
    elementsButtonRemove: function elementsButtonRemove(id) {
      ...
      _this.currentRowInput.val(JSON.stringify(current_value)); // Trigger change in the customizer field.


      if (triggerChange) {
        // Desktop.
        _this.currentRowInput.trigger('change'); // Mobile.
          ...
          var _loop4 = function _loop4() {
            ...
            wp.customize(optionID + deviceSelector, function (option) {
              option.bind(function (to) {
                ...
                _this.addBuilderRowColumnsClass(device, rowSelector, to); // Update row input value.


                var current_value = _this.jsonDecode($rowInput.val()); // Add column.
                ...
                $rowInput.val(JSON.stringify(current_value)); // Update the respective row columns layout customizer field.

                _this.updateColumnsLayoutOption(device, to); // Update 'Available Columns' area.


                _this.updateAvailableColumnsArea(device, to); // Trigger change in the customizer field (desktop).


                $rowInput.trigger('change'); // Trigger change in the customizer field (mobile).
                ...
                if (is_active) {
                  ...
                  var _loop6 = function _loop6() {
                    ...
                    setTimeout(function () {
                      ...
                      _this.currentRow = row;
                      _this.currentRowInput = $('#_customize-input-botiga_' + _this.currentBuilderType + '_row__' + row + '_' + _this.currentBuilderType + '_row'); // Update builder row columns class.

                      _this.addBuilderRowColumnsClass(device, rowSelector, wp.customize(columnsOptionID).get()); // Update 'Columns Layout' options.


                      _this.updateColumnsLayoutOption(device, wp.customize(columnsOptionID).get()); // Update 'Available Columns' area.
                  ...
                  for (var _i8 = 0, _devices2 = devices; _i8 < _devices2.length; _i8++) {
                    _loop6();
                  }
                }
              });
    ...
    updateColumnsLayoutOption: function updateColumnsLayoutOption(device, val) {
      var _this = this,
          ...
          selector = setting_id + '-' + wp.customize(setting_id).get(); // Hide the column layout options that doesn't match with 'columns' value.
    ...
    builderColumnsLayout: function builderColumnsLayout() {
      ...
      options.forEach(function (optionID) {
        if (typeof wp.customize.control(optionID) !== 'undefined') {
          ...
          var _loop7 = function _loop7() {
            ...
            wp.customize(optionID + deviceSelector, function (option) {
              option.bind(function (to) {
                ...
                _this.currentRowInput.val(inputValue).trigger('change'); // Trigger change on mobile row field.
          ...
          for (var _i9 = 0, _devices3 = devices; _i9 < _devices3.length; _i9++) {
            _loop7();
          }
        }
      });
    ...
    footerCustomizerOptions: function footerCustomizerOptions() {
      ...
      for (var _i10 = 0, _rows4 = rows; _i10 < _rows4.length; _i10++) {
        var row = _rows4[_i10];
        var fieldID = 'botiga_footer_row__' + row + '_footer_row'; // Vertical Aligment.
    ...
    updateHeaderPreset: function updateHeaderPreset(preset) {
      var _this = this,
          ...
          $below_row = $('#_customize-input-botiga_header_row__below_header_row'); // Set some others customizer settings.
      ...
      wp.customize('botiga_header_row__main_header_row_column3_horizontal_alignment_mobile').set('end'); // Set row settings and trigger change.
      ...
      $below_row.val(botiga_hfb.header_presets[preset]['below_row']).trigger('change'); // Trigger change on mobile row field.
