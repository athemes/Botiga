...
test('Home - No PHP Errors', async ({ page }) => {
	await page.goto('http://localhost:6066/');
	await expect(page.locator( 'html' )).not.toContainText(/(Fatal error:|Warning:|Notice:)/);
});

test('Shop Catalog - No PHP Errors', async ({ page }) => {
	await page.goto('http://localhost:6066/?post_type=product');
	await expect(page.locator( 'html' )).not.toContainText(/(Fatal error:|Warning:|Notice:)/);
});

test('Single Product - No PHP Errors', async ({ page }) => {
	await page.goto('http://localhost:6066/?product=deep-sweep-2-bha-pore-cleaning-toner-with-moringa');
	await expect(page.locator( 'html' )).not.toContainText(/(Fatal error:|Warning:|Notice:)/);
});

test('Cart (empty) - No PHP Errors', async ({ page }) => {
	await page.goto('http://localhost:6066/?page_id=28');
	await expect(page.locator( 'html' )).not.toContainText(/(Fatal error:|Warning:|Notice:)/);
});
...
test('My Account (logged-in) - No PHP Errors', async ({ page }) => {
	test.slow();
	await page.goto('http://localhost:6066/wp-admin/');

	// Login to admin
	await adminLoginAction( page );

	await page.goto('http://localhost:6066/?page_id=30');
	await expect(page.locator( 'html' )).not.toContainText(/(Fatal error:|Warning:|Notice:)/);
});
...
test('Checkout (cart empty) - No PHP Errors', async ({ page }) => {
	await page.goto('http://localhost:6066/?page_id=29');
	await expect(page.locator( 'html' )).not.toContainText(/(Fatal error:|Warning:|Notice:)/);
});
...
test('Search Page - No PHP Errors', async ({ page }) => {
	await page.goto('http://localhost:6066/?s=a&post_type=product');
	await expect(page.locator( 'html' )).not.toContainText(/(Fatal error:|Warning:|Notice:)/);
});

test('404 Page - No PHP Errors', async ({ page }) => {
	await page.goto('http://localhost:6066/?page_id=404notfound');
	await expect(page.locator( 'html' )).not.toContainText(/(Fatal error:|Warning:|Notice:)/);
});

test('Blog - No PHP Errors', async ({ page }) => {
	await page.goto('http://localhost:6066/?page_id=393');
	await expect(page.locator( 'html' )).not.toContainText(/(Fatal error:|Warning:|Notice:)/);
});

test('Blog Post - No PHP Errors', async ({ page }) => {
	await page.goto('http://localhost:6066/?p=119');
	await expect(page.locator( 'html' )).not.toContainText(/(Fatal error:|Warning:|Notice:)/);
});
...
test('Customizer - No PHP Errors', async ({ page }) => {
	test.slow();
	await page.goto('http://localhost:6066/wp-admin/customize.php');
	await adminLoginAction( page );
	await expect(page.locator( 'html' )).not.toContainText(/(Fatal error:|Warning:|Notice:)/);
});
...
test('Appearance > Menus - No PHP Errors', async ({ page }) => {
	test.slow();
	await page.goto('http://localhost:6066/wp-admin/nav-menus.php');
	await adminLoginAction( page );
	await page.waitForLoadState( 'networkidle' );
	await expect(page.locator( 'html' )).not.toContainText(/(Fatal error:|Warning:|Notice:)/);
});