...
function botiga_ajax_search_callback() {
    ...
    if( $qry->have_posts() ) :
        $output .= '<h2 class="botiga-ajax-search__heading-title">'. __( 'Products', 'botiga' ) .'</h2>';
    ...
    if( count( $cats ) > 0 && $search_term ) {
        $output .= '<h2 class="botiga-ajax-search__heading-title">'. __( 'Categories', 'botiga' ) .'</h2>';
        $output .= '<hr class="botiga-ajax-search__divider">';
    ...
    if( $output ) {
        ...
        $output = '<p class="botiga-ajax-search__no-results">'. __( 'No products found.', 'botiga' ) .'</p>';
